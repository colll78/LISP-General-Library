...
; the expressions to be evaluated from left to right. Additionally in
; the context of an (if ()..) expression, it allows multiple expressions
; to be evaluated in a conditional branch, in this case the branch that
; executes when the objlist is not empty.
...
; elements are added in the propeer order.
